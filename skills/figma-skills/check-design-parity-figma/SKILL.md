---
name: check-design-parity-figma
...
1. **Assemble the codeSpec.** Gather the values your component renders into the `codeSpec` object
   ...
   `scan-code-accessibility-figma --map-to-codespec` builds `codeSpec.accessibility` automatically.
2. **Read the design specs.** Set `NODE_ID` and paste your `codeSpec` into
   ...
   (`skillNames: "check-design-parity-figma"`). It extracts the node's fills, strokes, corner radius,
